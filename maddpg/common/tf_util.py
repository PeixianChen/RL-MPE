...
def save_state(fname, t,saver=None):
    """Save all the variables in the current session to the location <fname>"""
    os.makedirs(os.path.dirname(fname), exist_ok=True)
    if saver is None:
        saver = tf.train.Saver()
    path = saver.save(get_session(), fname+"simple_comm_", global_step=t)
    ...
    def __call__(self, *args, **kwargs):
        ...
        for inpt in self.inputs[len(args):]:
            ...
            if inpt_name in kwargs:
                ...
                if not inpt in self.givens:
                    print([i.name for i in self.inputs], len(args))
                    print(self.givens)
