...
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        pg_loss = -tf.reduce_mean(q * advantages)
        ...
        train = U.function(inputs=obs_ph_n + act_ph_n + [advantages], outputs=loss, updates=[optimize_expr])
...
class MADDPGAgentTrainer(AgentTrainer):
    ...
    def update(self, agents, t):
        ...
        for i in range(num_sample):
            ...
            advantages += rew + self.args.gamma * (1.0 - done) - target_q_next
        target_q /= num_sample
        advantages/=num_sample
        q_loss = self.q_train(*(obs_n + act_n + [target_q]))

        # train p network
        p_loss = self.p_train(*(obs_n + act_n + [advantages]))
